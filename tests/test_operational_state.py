...
class FakeGraph:
    ...
    def execute_command(self, command, graph, query, *args):
        ...
        if command == "GRAPH.QUERY" and "MERGE" in query:
            ...
            fenced = "r.expires_at_ms <= $now" in query
            if fenced and existing and existing[9] > params["now"]:
                if existing[4] != params.get("owner") or existing[5] > params["epoch"]:
                    return [[], [], []]
            row = [
                params["record_id"],
                params["kind"],
            ...
            if fenced:
                return [["r.record_id", "r.created_at_ms"], [[row[0], created]], []]
        ...
        if command == "GRAPH.QUERY" and "DELETE" in query:
            if "record_id" in params:
                removed = int(params["record_id"] in self.rows)
                self.rows.pop(params["record_id"], None)
                return [["count(r)"], [[removed]], []]
            expired = [
                key
                for key, row in self.rows.items()
                if row[9] <= params["now"]
            ]
            selected = expired[: params["limit"]]
            for key in selected:
                del self.rows[key]
            return [["count(r)"], [[len(selected)]], []]
        raise AssertionError((command, query, params))


def test_lease_is_atomic_bounded_and_epoch_monotonic():
    clock = [1_000]
    store = OperationalStateStore(FakeGraph(), clock_ms=lambda: clock[0])
    first = store.acquire_lease(
        logical_id="fleet-primary",
        owner="beelink",
        epoch=7,
        ttl_seconds=20,
    )
    ...
    assert renewed.epoch == 8


def test_expired_fence_can_be_acquired_by_new_owner():
    clock = [1_000]
    store = OperationalStateStore(FakeGraph(), clock_ms=lambda: clock[0])
    store.acquire_lease(
        logical_id="fleet-primary",
...
def test_finalization_commit_identity_is_deterministic():
    graph = FakeGraph()
    store = OperationalStateStore(graph, clock_ms=lambda: 1_000)
    record = store.upsert(
        kind="delegation",
        logical_id="task-1",
        state="ACTIVE",
    ...
    for _ in range(2):
        store.finalize(
            record,
            final_state="COMPLETED",
            neo4j_commit=lambda envelope: commits.append(envelope),
